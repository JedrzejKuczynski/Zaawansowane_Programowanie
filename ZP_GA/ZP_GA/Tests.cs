...
namespace ZP_GA
{
    class Tests
    {
        List<int> fragments;
        List<int> samples;
        List<Tuple<int, int>> matrix_sizes;
        List<double> fills;
        ...
        public Tests()
        {
            ...
            matrix_sizes = new List<Tuple<int, int>> { Tuple.Create(20, 20), Tuple.Create(25, 25) };
            fills = new List<double> { 0.1, 0.3, 0.5 };
            // Błędy dynamicznie CO Z NIMI??? OTO PYTANIE


            // Heurystyka

            pop_sizes = new List<int> { 50, 100, 200, 300, 400, 500, 700, 1000 };
                        ...
                        for (int e = 0; e < errors.Count; e++)
                            for (int p = 0; p < pop_sizes.Count; p++)
                            {
                                ...
                                if (pop_sizes[p] <= current_samples * 10)
                                {
                                    ...
                                    List<int> tournament_sizes = new List<int> { Convert.ToInt32(pop_size*0.1),
